...
/////Below is the display block that receives the data from arguments will callbacks
/*getOps.displayCallback = function(err, resp, body){
...
var back = new ValidResponse(['b', 'back']);
var confirm = new ValidResponse(['y', 'yes']);

var curPage = 1;
function waiting(){
  var text = 'Loading...: '+ back.print();
  prompt.pause();
  prompt.get(text, function(err, result){
    var response = result[text];
    if(back.isValid(response)){
      stack();
    } else {
      waiting();
    }
...
function questionSelection(answers) {
      ...
      answers.items.forEach(function(item, index){
        console.log('['+clc.cyan(index)+']: ' + clc.yellow(htmlToText.fromString(item.title)) + '\n');
      });
      ...
      prompt.get(message, function(err, result) {
        ...
        } else if(getOps.checkAlt(response)){
          waiting();
        } else if (exit.isValid(response)) {
          console.log('Quitting');
